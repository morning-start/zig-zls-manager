...
/// 同步 Zig 和 ZLS 到推荐组合
pub async fn cmd_sync(ctx: &AppContext, _dry_run: bool, json: bool) -> Result<(), ZzmError> {
    ...
    // 消息输出回调（不依赖 manager 的 callbacks）
    let on_success = if json {
        |_: &str| {}
    } else {
        |msg: &str| console_output::print_success(msg)
    };
    let on_info = if json {
        |_: &str| {}
    } else {
        |msg: &str| console_output::print_info(msg)
    };
    let on_warning = if json {
        |_: &str| {}
    } else {
        |msg: &str| console_output::print_warning(msg)
    };
